...
function getRioApiUrl() {
  const baseUrl = cleanOptionalValue(API_BASE_URL);

  if (!baseUrl) {
    throw new Error("Rio registration API is not configured.");
  }

  return `${baseUrl.replace(/\/+$/, "")}${RIO_REGISTER_PATH}`;
}
...
export async function registerRioLead(formValues) {
  ...
  const response = await fetch(getRioApiUrl(), {
    method: "POST",
    headers: {
